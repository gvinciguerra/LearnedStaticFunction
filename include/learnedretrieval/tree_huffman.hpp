...
public:
    ...
    explicit TreeHuffman(const std::vector<std::pair<Symbol, Frequency>> &input) { initialize(input); }
    ...
    explicit TreeHuffman(const FrequenciesVector &input) {
    ...
    ~TreeHuffman() { delete_tree(root); }

    TreeHuffman(const TreeHuffman &) = delete;

    TreeHuffman &operator=(const TreeHuffman &) = delete;

    TreeHuffman(TreeHuffman &&) = delete;

    TreeHuffman &operator=(TreeHuffman &&) = delete;
